...
void AES_Encrypt_B16(BYTE block[], BYTE key[], int keyLen) {
  int l = keyLen, i;
  //printBytes(block, 16);
...
void AES_Decrypt_B16(BYTE block[], BYTE key[], int keyLen) {
  int l = keyLen, i;
...
int aes_init(unsigned char *key, unsigned int key_len) {
  AES_Init();
  memcpy(g_key, key, key_len * sizeof(BYTE));
  g_key_len = key_len;
  g_expand_key_len = AES_ExpandKey(g_key, key_len);
  return 0;
}

int aes_encrypt(unsigned char *src, unsigned int src_len, unsigned char *dest) {
  if (dest != src) {
    memcpy(dest, src, src_len * sizeof(unsigned char));
  }
  for (int i = 0; i + 16 <= src_len; i += 16) {
    AES_Encrypt_B16(dest + i, g_key, g_expand_key_len);
  }
  return 0;
}
...
int aes_decrypt(unsigned char *src, unsigned int src_len, unsigned char *dest) {
  ...
  for (int i = 0; i + 16 <= src_len; i += 16) {
    AES_Decrypt_B16(dest + i, g_key, g_expand_key_len);
  }
  return 0;
}
